...
RUN wget -qO- https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor -o /tmp/microsoft-ar

# Move the GPG key to the appropriate directory
RUN sudo mv /tmp/microsoft-archive-keyring.gpg /usr/share/keyrings/

RUN wget https://packages.microsoft.com/config/ubuntu/20.04/packages-microsoft-prod.deb -O /tmp/pack
